...
BEGIN
    IF NOT EXISTS (SELECT * FROM countries WHERE id = NEW.country_id)
		THEN
        SET @message = CONCAT('TRIGGER - Cannot insert info because country ID:', NEW.country_id, ' does exist');
...
BEGIN
    IF NOT EXISTS (SELECT * FROM countries WHERE id = NEW.country_id)
		THEN
        SET @message = CONCAT('TRIGGER - Cannot update info because country ID:', NEW.country_id, ' does exist');
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = @message;
END IF;
END //
DELIMITER ;
...
BEGIN
    IF EXISTS (SELECT * FROM country_info WHERE country_id = OLD.id)
		THEN
        SET @message = CONCAT('TRIGGER - Cannot update country _inwith ID:', OLD.id, ' because it is used in: "countryfo"');
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = @message;
END IF;
END //
DELIMITER ;
